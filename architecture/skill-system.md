...
|---------|---------------|------|
| 上下文 | 共享主对话 | 独立子 Agent |
| Token 预算 | 与主对话共享 | 有自己的预算（maxTurns=30） |
| 历史可见性 | LLM 能看到之前的对话 | 看不到主对话历史 |
| 结果处理 | 直接在当前对话继续 | 文本结果返回主对话 |
| 适用场景 | 大多数 Skill | 独立分析、搜索、审计等任务型 Skill |
| 当前支持 | **已实现** | **已实现** |
...
Fork 模式通过 `ForkExecutor` 类实现，执行流程如下：

```mermaid
flowchart TD
    ST["SkillTool.execute()"] --> CHK{"command.isFork()?"}
    CHK -->|No| INLINE["Inline 模式<br/>渲染内容作为 ToolResult"]
    CHK -->|Yes| FORK["ForkExecutor.execute()"]
    
    FORK --> SP["构建子 Agent 系统提示词<br/>（简化版，不含 Skill 列表）"]
    SP --> TR["构建子 Agent 工具集<br/>（按 allowedTools 过滤）"]
    TR --> CREATE["创建子 AgentLoop<br/>maxTurns=30, commandRegistry=null"]
    CREATE --> BANNER1["终端显示 ┌─ Fork: skillName ─┐"]
    BANNER1 --> RUN["childLoop.run(renderedPrompt)<br/>子 Agent 独立执行"]
    RUN --> BANNER2["终端显示 └─ Fork: skillName ── complete ─┘"]
    BANNER2 --> RESULT["返回子 Agent 最终文本作为 ToolResult"]
```

**关键设计决策**：
- 子 Agent 的 `commandRegistry = null`：从根本上杜绝递归 fork
- 共享 `PermissionManager`：用户在主对话批准的操作，子 Agent 不再重复询问
- 共享 `ClaudeApiClient`：复用同一个 HTTP 连接
- 独立 `ConversationHistory`：子 Agent 的中间过程不污染主对话上下文

### 什么 Skill 适合 Fork？

| 类型 | 模式 | 特征 | 例子 |
|------|------|------|------|
| **指令型** | inline | 告诉 LLM "怎么做" | pdf、commit、claude-api |
| **任务型** | fork | 交给子 Agent 独立完成 | 代码搜索、依赖分析、安全审计 |

在 SKILL.md 中声明 `context: fork` 即可启用 Fork 模式：
